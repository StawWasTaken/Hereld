...
(function () {
  ...
  /* Every write lands here, so the log line, the toast and the repaint are the
     same three steps everywhere and cannot drift apart. */
  ...
  /* One account, everything known about it, and every action that applies. */
  ...
  /* Suspending without a reason produces a log that explains nothing later. */
  ...
  async function pageBots(host) {
    ...
    node.innerHTML =
      ...
      '<form class="hd-stf-search" id="stfBotN">' +
        ...
        /* No ceiling. This is how many accounts may take part, not how many
           exist, so it can be set before any are made and the count simply
           takes effect as they are. A max tied to the number that exist
           locks the field shut while none do. */
  ...
  /* A page the role cannot reach is not drawn, and the database would refuse
     it anyway. Both checks exist because only one of them is a security
